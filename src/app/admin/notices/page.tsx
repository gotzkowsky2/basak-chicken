                ...
                <div key={n.id} className="p-4 hover:bg-gray-50">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="ml-3 flex gap-2">
                      ...
                      <button onClick={async()=>{ if(!confirm('정말 비활성화 하시겠습니까?')) return; const r=await fetch(`/api/admin/notices/${n.id}`,{ method:'DELETE', credentials:'include' }); if(r.ok){ fetchList(); } }} className="px-3 py-1 bg-red-700 text-white rounded">삭제</button>
                    </div>
                  </div>
                </div>
              ))}
